	...
	return (
		...
		>
			<SwitchPrimitive.Thumb
				...
				className={cn(
					'bg-primary data-[state=checked]:bg-card dark:data-[state=unchecked]:bg-foreground dark:data-[state=checked]:bg-primary-foreground pointer-events-none block size-4 rounded-full ring-0 transition-transform data-[state=checked]:translate-x-[calc(100%-2px)] data-[state=unchecked]:translate-x-0',
				)}
			/>
		</SwitchPrimitive.Root>
	);
...
export const CompleteFormFieldSwich = <T extends object>({
	control,
	name,
	label,
}: {
	...
	return (
		<FormField
			...
			name={name}
			render={({ field }) => (
				<FormItem className="flex flex-row items-center py-2">
					<FormControl>
